/* BFD back-end for ix86 OMF objects.
   ...
   Edited by ChrisNonyminus to work with modern binutils.
...
// Caveat: only supports 32-bit OMFs
...
#define OMF_COMENT_EASYOBJ             0xaa
...
#define OMF_COMENT_LINKER_DRCTV        0xfe
...
i386omf_read_pubdef (bfd *abfd, bfd_byte const *p, bfd_size_type reclen)
{
  ...
  if (base_segment == OMF_PUBDEF_SEGMENT_ABSOLUTE)
      ...
      if (reclen < 2)
	{
      ...
      base_frame = bfd_get_32 (abfd, p);
      p += 4; // TODO: support for 16-bit
      reclen -= 4;
  ...
  while (reclen)
      ...
      else
	{
      ...
      if (!i386omf_read_offset (abfd, &seglen, &p, &reclen,
				use32 ? I386OMF_OFFSET_SIZE_32 : I386OMF_OFFSET_SIZE_16))
	return FALSE;
      ...
      if (alignment == OMF_SEGDEF_ALIGNMENT_UNDEFINED)
	{
	  _bfd_error_handler ("Segment %d (%s) wants alignment = 7",
				 name_index,
				 (char*)strtab_lookup (tdata->lnames, name_index));
	  bfd_set_error (bfd_error_wrong_format);
	  return FALSE;
	}
  ...
  while (reclen)
      ...
      if (subrec & OMF_FIXUPP_FIXUP)
	{
	  ...
	  switch (frame_method)
		...
		frame = use32 ? (int) bfd_get_32(abfd, p):  (int) bfd_get_16 (abfd, p);
		p += use32 ? 4 : 2;
		reclen -= use32 ? 4 : 2;
	  ...
	  switch (target_method)
		...
		target =use32 ? (int) bfd_get_32(abfd, p):  (int) bfd_get_16 (abfd, p);
		p += use32 ? 4 : 2;
		reclen -= use32 ? 4 : 2;
		target_relent->symbol = NULL;
		break;
			    ...
			    bfd_size_type reclen)
{
  ...
  if (block_count == 0)
	...
	{
	  _bfd_error_handler ("LIDATA at 0x%lx overflows section %ld",
				 asect->filepos, p - tdata->image);
	  bfd_set_error (bfd_error_wrong_format);
	  return 0;
	}
      ...
      while (block_count--)
	{
      ...
      if (reclen)
	_bfd_error_handler ("Leftover LIDATA at 0x%lx in section %ld",
			       asect->filepos, p - tdata->image);
    }
			  ...
			  bfd_size_type reclen, int rectype)
{
  ...
  if ((asect->flags & SEC_IN_MEMORY) == 0)
	...
	{
	  _bfd_error_handler ("Out of memory for %x section contents",
				 asect->id);
	  return FALSE;
	}
  ...
  if (rectype & (OMF_RECORD_LEDATA ^ OMF_RECORD_LIDATA))
	...
	{
	  _bfd_error_handler ("LEDATA at 0x%lx overflows section %ld",
				 asect->filepos, p - tdata->image);
	  bfd_set_error (bfd_error_wrong_format);
	  return FALSE;
	}
		      ...
		      bfd_size_type reclen, int rectype)
{
  ...
  if (!i386omf_read_offset (abfd, &offset, &p, &reclen,
			   segdef->use32 ? I386OMF_OFFSET_SIZE_32 : I386OMF_OFFSET_SIZE_16))
    return FALSE;
...
i386omf_teardown_tdata (bfd *abfd)
{
  ...
  bfd_realloc_or_free(tdata->image, 0);
}
...
//     return FALSE;
  tdata->image = bfd_malloc(osize);
  if (bfd_bread(tdata->image, osize, abfd) != osize)
      return FALSE;
			   ...
			   reclen ? reclen - 1 : 0, p + OMF_RECORD_HEADER))
	{
	  switch (bfd_get_error ())
		...
		_bfd_error_handler ("process_record() failed at 0x%lx",
				       p - tdata->image);
		hexdump (0, reclen + OMF_RECORD_HEADER, p);
...
void i386omf_cleanup_generic (bfd *abfd);
...
i386omf_object_p (bfd *abfd)
{
  ...
  struct i386omf_obj_data* tdata = bfd_alloc (abfd, sizeof (struct i386omf_obj_data));
  if (!tdata)
    return NULL;
  ...
  return i386omf_cleanup_generic;
}

void
i386omf_cleanup_generic (bfd *abfd)
{
  i386omf_teardown_tdata (abfd);
}
...
#define i386omf_set_reloc _bfd_generic_set_reloc
...
#define i386omf_get_symbol_version_string _bfd_nosymbols_get_symbol_version_string
